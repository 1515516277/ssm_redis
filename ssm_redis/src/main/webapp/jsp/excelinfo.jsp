...
<table class="table table-bordered">
    <tr>
        <td colspan="17">公司：<c:if test="${sheets.size()>0}">${sheets.get(0).corpName}</c:if></td>
    ...
    <c:forEach items="${sheets}" var="s" varStatus="status">
        <tr>
            <td>${s.subjectName}</td>
            <td>${status.count}</td>
            <td>${s.beginYearBalance<=0?'':s.beginYearBalance}</td>
            ...
            <td>${s.getval(month)-(month>1?s.getval(month-1):0)}</td>
            <td>${s.getval(month)-s.beginYearBalance}</td>
            <td>
