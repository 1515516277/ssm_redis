 ...
 * */
@Controller
@RequestMapping("/sheetlr")
public class SheetLrController {
    ...
    public String sheet(Model model, String company, String time, String month) {
        try {
            ...
            sheet.setYears(time);
            List<SheetEntity> sheetEntities = sheetservice.selsheet01_lr(sheet);
    ...
    public Msg editanalyze(String month, int id, String val) {
        try {
            ...
            sheet.setId(id);
            sheetservice.editsheet_lr(sheet);
            return Msg.success();
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public String sheet1(@Param("file") MultipartFile file, String company, String time, String month, HttpServletRequest request) {
        try {
            ...
            sheet.setCorpName(company);
            int i = sheetservice.selsheet01_lr(sheet).size();
            InputStream in = null;
            if (file.isEmpty()) {
                throw new Exception("文件不存在");
            }
            in = file.getInputStream();
            listob = ExcelUtil.sheet02(in, file.getOriginalFilename(), company, time, month);
            in.close();
            if (i > 0) {
                for (SheetEntity li : listob) {
                    sheetservice.editsheet_lr(li);
                }
            } else {
                sheetservice.inserts_lr(listob);
            }

        } catch (Exception e) {
            e.printStackTrace();
        }
        return "redirect:/sheetlr/sheetinit";
    }
    ...
    public void sheetout(String company, String time, String month, HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            sheet.setCorpName(company);
            sheet.setYears(time);
            List<SheetEntity> list = sheetservice.selsheet01_lr(sheet);
            if (list.size() > 0) {
